...
def previous_date(date_list):
    if '-' in date_list[0]:
        ...
        previous_dates = [datetime.strptime(date_str, "%Y-%m-%d") - timedelta(days=1) for date_str in date_list]
        # Convert datetime objects back to formatted date strings
        previous_date_strings = [date_obj.strftime("%Y-%m-%d") for date_obj in previous_dates]
...
def fine_ana(df_ss, CT_info):
    ...
    ClinicTrial = []
    verifiedCT = []
    df_ss = df_ss.sort_values(by=['directRelAll', 'finalProb', 'finalScore'], ascending=False)
    ...
    df_ss = df_ss.sort_values(by=['finalProb', 'finalScore', 'pathCnt'], ascending=False)
    df_ss.reset_index(drop=True, inplace=True)
    df_ss['Verified_allSource'] = [1 if df_ss.ClinicTrial.tolist()[i]==1 or df_ss.directRelAll.tolist()[i]==1 else 0 for i in range (len(df_ss))]
...
def initial_directCD(verify_start_date):    # time1 = '20200401'
    ...
    directCD = {'Time':date_list, 'verified_repDrug':[0 for i in range (len(date_list))], \
...
def Drug_Verify(df, CT_info, verify_start_date, MIN_PROB):
    ...
    df_sub = df.loc[(df['Drug']==1)&(df['directRel']==0)]   # get the drugs that doesn't have direct relation before repurpose
    # remove the drugs that verified by clinical trial before the verify_start_date
    remove_byCT = []
    for i, row in df_sub.iterrows():
        if row['chem'] in CT_info:
            if datetime.strptime(CT_info[row['chem']]['time'].split()[0], "%Y-%m-%d")<verify_start_date_obj:
                remove_byCT.append(i)
    df_sub_d = df_sub.drop(remove_byCT)
    df_sub = df_sub_d.copy()
    df_sub = df_sub.sort_values(by=['directRelAll', 'finalProb', 'finalScore'], ascending=False)
    df_sub.reset_index(drop=True, inplace=True)
    df_sub['ClinicTrial'] = [1 if df_sub.chem.tolist()[i] in CT_info else 0 for i in range (len(df_sub))]
    df_sub['Verified_allSource'] = [1 if df_sub['directRelAll'].tolist()[i]==1 or df_sub['ClinicTrial'].tolist()[i]==1 else 0 for i in range (len(df_sub))]
    ...
    for Ti in range (1, len(directCD['Time'])):
        ...
        time2 = directCD['Time'][Ti]    # the time2 is the end date for PubMed and Clinical verification
        for ind, row in df_sub.iterrows():
            ...
            if drugID in CT_info:
                if set(CT_info[drugID]['CT_id']).intersection(set(verifiedCT)):
                    ...
                    timeCT_t = datetime.strptime(CT_info[drugID]['time'].split()[0], "%Y-%m-%d")
                    if timeCT_t < verify_start_date_obj:    # the drug verified by clinical trial before the verify start date should not be count 
                        remove_index.append(ind)
                        verified_list.append(drugID)
                        continue
                ...
                if relID in RELlist:
                    ...
                    if finalProb != 0:
                        if relID_op in RELlist:
                            rels_op = get_rel_within_time(RELlist[relID_op], time0, time2)
                            finalProb_op, sumscore_op = get_prob_fromArtList(rels_op, MIN_PROB)
                        else:
                            finalProb_op = 0
                            sumscore_op = 0
...
if __name__ == "__main__":
    ...
    files = glob.glob('DrugRepurposing_result_v3/*.json')
    sorted_files = sorted(files, key=lambda x: datetime.strptime(x.split('.')[1].split('_')[1], '%Y%m%d'))
    for p in [1.00]:
        summary = {'time':[], 'N_totDrug':[], 'N_inClinicTrial':[], 'N_inPubMed':[], 'Drug_verified':[], 'N_gene':[]}
        for F in sorted_files[:6]:
            data = read_json(F)
            if data == []:
                continue
            for x in data:
            ...
            directCD, df_ss = Drug_Verify(df_ss, CT_info, verify_start_date, MIN_PROB_direct)
            ...
            if df_ss.Verified_allSource.sum() != sum(directCD['verified_repDrug']):
                print(F, df_ss.Verified_allSource.sum() , sum(directCD['verified_repDrug']))
            if p == 1:
                df_ss.to_csv(output_fold+verify_start_date+'.csv', index=False)
                directCD_df = pd.DataFrame(directCD)
                directCD_df.to_csv(output_fold+verify_start_date+'_direct.csv', index=False)
