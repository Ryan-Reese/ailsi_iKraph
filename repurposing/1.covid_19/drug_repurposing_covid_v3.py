...
if __name__ == "__main__":
    ...
    output_path = 'DrugRepurposing_result_v3/' 
    ...
    target_list = []    # [[entID, ent_type, cutoff_time1, cutoff_time2], ...] ent_type='C' or 'D', cutoff_time='YYYY-MM-DD'
    target_entName = {}    # {entID:entity_name} entity_name='common name' or 'official name'
    if input_list != "":
        with open(input_list,'r') as f:
            Entity_list = [x.split() for x in f if not x.startswith('#')]    #[['Alzheimer's Disease', 'D', 'AD', '3651', '1990-01-01', '2023-08-21'], ...]
        for ent in Entity_list:
            temp = [ent[3], ent[1], ent[-2], ent[-1]]   # type (D or C), time_cut1, time_cut2
            if ent[1] == 'C' and entDic[ent[3]] != "Chemical":
                print('Chemical ID provided is not in the database:', ent[3], flush=True)
    ...
    for rel in RELlist:
        ...
        if entDic[n1] == 'Chemical' and entDic[n2] == 'Disease' and direction == '12':
            rels = get_rel_within_time(RELlist[rel], '2020-01-01', '2100-01-01')  # [[pmid, prob, score, time_object],...], early time cutoff specially for COVID-19
    ...
    for target in target_list:
        ...
        time1 = target[2]
        time2 = target[3]
        print(f"Drug repurposint:{targetType} {target_entName[targetID]} ({targetID}), Time: {time1}-{time2}", flush=True)
        ...
        for rel in RELlist:
            ...
            if entDic[n1] == 'Chemical' and entDic[n2] == 'Gene' and direction == '12':
                ...
                if targetType == 'D':   # target entity is disease, so there is no cut_time1 for chemical-gene, set cut_time1 as a early date
                    rels = get_rel_within_time(RELlist[rel], '1000-1-1', time2)  # [[pmid, prob, score, time_object],...]
            ...
            if entDic[n1] == 'Disease' and entDic[n2] == 'Gene' and direction == '21':
                # calculate probability
                if targetType == 'C':   # target entity is chemical, so there is no cut_time1 for disease-gene, set cut_time1 as a early date
                    rels = get_rel_within_time(RELlist[rel], '1000-1-1', time2)  # [[pmid, prob, score, time_object],...]
        ...
        if targetType == "D":   # process for drug repurposing for disease
                ...
                for gene in ChemGeneDic[chem]:
                    ...
                    if gene in GeneDisDic:
                        if targetID in GeneDisDic[gene]:
                            ...
                            key_op = chem+'.'+targetID + '.' + reverse_corType(cdTypeStr)
            ...
            outputfile = output_path + targetID + '.' + time1.replace("-","") +'_'+time2.replace("-","") +'.json'
    ...
    with open(output_path + target_list[-1][0] + '.' + target_list[-1][2].replace("-","") +'_'+target_list[-1][3].replace("-","") +'.json', 'r')as f:
        candidate = json.load(f)
    cand_chem = set([x['chem'] for x in candidate])
    directCD = {'Time':[], 'allCD':[], 'allDrugD':[],  'repCD':[], 'repDrugD':[]}

    # read ChemEntity
    with open('/home/yuan/11.KG/drug_repurposing/1.dev/data/ChemEntities.json', 'r') as f:
        Chem = json.load(f)
    ChemDict = {}
    for C in Chem:
    ...
    for target in target_list:
        ...
        for rel in RELlist:
            # relation id rule: cell line - chemical - disease - gene - mutations - species
            n1, n2, corType, direction = rel.split('.')
            if n2 == targetID and entDic[n1] == 'Chemical' and direction == '12' and corType == '0':
