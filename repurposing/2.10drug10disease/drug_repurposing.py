...
def repurpose_fromC(RELlist, entDic, entSubtype, entName, targetID, MIN_PROB, MIN_PROB_direct, COR_TYPE, time1, time2):
        ...
        if n1 == targetID and int(corType)-1 == COR_TYPE:
            ...
            if dirProb - dirProb_op > 0.1:
                if n1 not in allCD_inT:
                    allCD_inT[n2] = [dirProb, dirScore]
                elif allCD_inT[n2][0] < dirProb:
                    allCD_inT[n2] = [dirProb, dirScore]
    return relDic, allCD_inT


def repurpose_fromD(RELlist, entDic, entSubtype, entName, targetID, MIN_PROB, MIN_PROB_direct, COR_TYPE, time1, time2):
    # get Chemical-Gene, Disease-Gene dict for repurposing; Get Chem-Disease dict for direct relation
    ChemGeneDic = {}
    GeneDisDic = {}
    ChemDisDic = {}
...
if __name__ == "__main__":
    ...
    output_path = 'DrugRepurposing_result' 
    ...
    if input_list != "":
        with open(input_list,'r') as f:
            Entity_list = [x[:-1].split('\t') for x in f if not x.startswith('#')]    #['baricitinib',C'],['ibuprofen','C'],...]
        target_D = [x[2].lower() for x in Entity_list if x[1]=='D']
        target_C = [x[2].lower() for x in Entity_list if x[1]=='C']
        if len(Entity_list[0])>2:
            for x in Entity_list:
                inputTargetID[x[2].lower()] = x[-1]     # {'name':'biokdeid'}
    else:
    ...
    with open(EntDictFile, 'r') as f:
        Edata = json.load(f)
    target_ID = {'D':{x:[] for x in target_D}, 'C':{x:[] for x in target_C}}  #{entity_name: entID} entity_name='common name' or 'official name'
    entDic = {}
    entSubtype = {}
    entName = {}
    for item in Edata:
        ...
        if item['type'] == 'Disease':
            if commonName.lower() in target_D:
                target_ID['D'][commonName.lower()].append(bkid)
            elif item['official name'].lower() in target_D:
                target_ID['D'][item['official name'].lower()].append(bkid)
        if item['type'] == 'Chemical':
            if commonName.lower() in target_C:
                target_ID['C'][commonName.lower()].append(bkid)
            elif item['official name'].lower() in target_C:
                target_ID['C'][item['official name'].lower()].append(bkid)
    ...
    finalizedTargetID_dict = {}
    for x in target_ID:
        print(x)
        for y in target_ID[x]:
            if inputTargetID != {}:
                if inputTargetID[y] in target_ID[x][y]:
                    finalizedTargetID_dict[y] = inputTargetID[y]
                else:
                    print(f'The input ID ({inputTargetID[y]}) of target {y} not match extracted {target_ID[x][y]}', flush=True)
                    finalizedTargetID_dict[y] = target_ID[x][y][0]
            else:
                print(y, target_ID[x][y])
                finalizedTargetID_dict[y] = target_ID[x][y][0]
    ...
    ChemDisDicAll = {}
    CD_time = {'D':{finalizedTargetID_dict[target]:[] for target in target_D}, \
               'C':{finalizedTargetID_dict[target]:[] for target in target_C}}
    for rel in RELlist:
        n1, n2, corType, direction = rel.split('.')
        if n1 not in entDic or n2 not in entDic:
            continue
            ...
            if key not in ChemDisDicAll:
                ...
                if finalProb > ChemDisDicAll[key][0]:
                    ...
                    if sumscore > ChemDisDicAll[key][2]:
                        ChemDisDicAll[key] = [finalProb, corType, sumscore]
        if mode == 'timely':
            if n1 in CD_time['D'] and entDic[n2] == 'Gene' and direction == '21':
                CD_time['D'][n1].extend([X[3] for X in RELlist[rel]])
            if n1 in CD_time['C'] and entDic[n2] == 'Gene' and direction == '12':
                CD_time['C'][n1].extend([X[3] for X in RELlist[rel]])

    # begin the repurposing for each target
    if target_C != []:
        for target in target_C:
            targetID = finalizedTargetID_dict[target]
            time1 = '1000-01-01'
            if mode == 'newest':
                time2 = '2023-01-01'
                relDic,ChemDisDicTarget = repurpose_fromC(RELlist, entDic, entSubtype, entName, targetID, MIN_PROB, MIN_PROB_direct, COR_TYPE, time1, time2)
                if relDic != {}:
                    outputfile = output_folder + '.'.join(['_'.join(target.split()), targetID, time1.replace("-",""), time2.replace("-",""), 'json'])
                ...
                for time2 in Time_list:
                    relDic,allCD_inT[time2] = repurpose_fromC(RELlist,entDic,entSubtype,entName, targetID, MIN_PROB, MIN_PROB_direct, COR_TYPE, time1, time2)
                    outputfile = output_folder + '.'.join(['_'.join(target.split()), targetID, time1.replace("-",""), time2.replace("-",""), 'json'])
                    if relDic != {}:
                        with open(outputfile, 'w') as f:
                            json.dump(list(relDic.values()), f, indent=2)
                ...
                for key in ChemDisDicAll:
                    ...
                    if n1 == targetID and int(corType)-1 == COR_TYPE:
                        ...
                        if K_op in ChemDisDicAll:
                            ...
                            dirProb_op = 0
                        if dirProb - dirProb_op > 0.1:
                            if n2 not in ChemDisDicTarget:
                                ChemDisDicTarget[n2] = [dirProb, dirScore]
                            else:
                                if ChemDisDicTarget[n2][0] < dirProb:
                                    ChemDisDicTarget[n2] = [dirProb, dirScore]
                outputfile = output_folder + '_'.join(target.split()) + '.' + targetID  +'.directTreatment.json'
                with open(outputfile, 'w') as f:
                    json.dump(ChemDisDicTarget, f, indent=2)
    if target_D != []:
        for target in target_D:
            ...
            if mode == 'newest':
                time2 = '2023-01-01'
                relDic,ChemDisDicTarget = repurpose_fromD(RELlist, entDic, entSubtype, entName, targetID, MIN_PROB, MIN_PROB_direct, COR_TYPE, time1, time2)
                if relDic != {}:
                    outputfile = output_folder + '.'.join(['_'.join(target.split()), targetID, time1.replace("-",""), time2.replace("-",""), 'json'])
                    with open(outputfile, 'w') as f:
                        json.dump(list(relDic.values()), f, indent=2)
                ...
                allCD_inT = {}  # {time:{disID:[prob,score],..}}
                for time2 in Time_list:
                    relDic,allCD_inT[time2] = repurpose_fromD(RELlist,entDic,entSubtype,entName, targetID, MIN_PROB, MIN_PROB_direct, COR_TYPE, time1, time2)
                    outputfile = output_folder + '.'.join(['_'.join(target.split()), targetID, time1.replace("-",""), time2.replace("-",""), 'json'])
                    if relDic != {}:
                        with open(outputfile, 'w') as f:
                            json.dump(list(relDic.values()), f, indent=2)
