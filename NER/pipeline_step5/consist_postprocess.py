...
def parse_argument():
    parser = argparse.ArgumentParser(
        description="Post Processing with Consistency Check"
    )
    parser.add_argument(
        "-i",
        "--input",
        default="predictions.txt",
        help="input prediction file, Default is 'predictions.txt'",
        type=str,
    )
    parser.add_argument(
        "-t",
        "--token",
        default="test.json",
        help="token info file in json format, Default is 'test.json'",
        type=str,
    )
    parser.add_argument(
        "--no_fulltext",
        help="doing consistency check NOT based on fulltext",
        action="store_true",
    )
    parser.add_argument(
        "--pred_ft",
        default="predictions_ft.txt",
        help="prediction file of fulltext, Default is 'predictions_ft.txt'",
        type=str,
    )
    parser.add_argument(
        "--token_ft",
        default="test_ft.json",
        help="token info file of fulltext in json format, Default is 'test_ft.json'",
        type=str,
    )
    parser.add_argument(
        "-o",
        "--output",
        default="predictions_ppConsist.txt",
        help="output prediction file, Default is 'predictions_ppConsist.txt'",
        type=str,
    )
...
for line in fjson[0]:
    ...
    for i in range(len(data["tokens"])):
        ...
        if phrase in label[pubid].keys() and label[pubid][phrase] != "NaN":
            if (
                not check_subphrase(data["ner_tags"], i, i)
                and data["ner_tags"][i].split("-")[-1] != label[pubid][phrase]
            ):
                print(
                    "Inconsistency fixed, line ",
                    ln,
                    "|",
                    phrase,
                    ":",
