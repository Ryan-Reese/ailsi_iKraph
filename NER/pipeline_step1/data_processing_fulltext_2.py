...
train_fulltext = json.load(open('data_files/fulltext/litcoin_train_fulltext.json', 'r', encoding='utf-8'))
test_fulltext = json.load(open('data_files/fulltext/litcoin_test_fulltext.json', 'r', encoding='utf-8'))

full_train_pmid = {v['pmid']:{"document_id": v['pmid'], "text": v['fulltext']} for v in train_fulltext.values()}
full_test_pmid = {v['pmid']:{"document_id": v['pmid'], "text": v['fulltext']} for v in test_fulltext.values()}
...
for k,v in full_train_pmid.items():
    doc_id = v['document_id']
    text = v['text']
    sents = sentence_tokenizer.tokenize(text)
    for idx,sent in enumerate(sents):
        record = {}
        record[str(doc_id)+'_'+str(idx)] = {"document_id": doc_id, 'id': idx, "sent": sent}
        if idx == 0:
...
for k,v in full_test_pmid.items():
    doc_id = v['document_id']
    text = v['text']
    sents = sentence_tokenizer.tokenize(text)
    for idx,sent in enumerate(sents):
...
with open("temp/full_train_pmid_sents.txt", 'w') as output: 
    for item in full_train_pmid_sents:
        for k,v in item.items():
            output.write(v['document_id']+'\t'+v['sent']+'\t'+str(v['sent_spans'][0])+'\t'+
                         str(v['sent_spans'][1])+'\n')
...
exec(open("sent_split_post_nltk_fulltext_2.py").read())
...
def get_tok_for_text(text, offset):
    ...
    for token in doc:
        for tk, start, end in split_punct(token.text, token.idx):
            ...
            spans.append(span)
    token_list.append(tokens); span_list.append(spans)
    return tokens, spans

full_train_pmid_tokens = []
i = 0
for item in full_train_pmid_sents_refined:
...
j = 0
for item in full_test_pmid_sents_refined:
    key = list(item.keys())[0]
    docid = item[key]['document_id']
    sent = item[key]['sent']
    offset = item[key]['sent_spans'][0]
...
json.dump(full_train_pmid_tokens, open("temp/full_train_pmid_tokens.json", "w", encoding="utf-8"))
json.dump(full_test_pmid_tokens, open("temp/full_test_pmid_tokens.json", "w", encoding="utf-8"))


### Labeling ###
full_train_bio = []
for record in full_train_pmid_tokens:
    ner_tags = []
...
full_test_bio = []
for record in full_test_pmid_tokens:
    ner_tags = []
    ner_tags.extend([*(len(record['tokens'])*['O'])])
    record['ner_tags'] = ner_tags
