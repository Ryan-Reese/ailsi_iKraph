...
for k,v in abstr_train_pmid.items():
...
for k,v in abstr_test_pmid.items():
    doc_id = v['document_id']
    text = v['text']
    sents = sentence_tokenizer.tokenize(text)
    for idx,sent in enumerate(sents):
...
i = 0
for item in abstr_train_pmid_sents_refined:
...
j = 0
for item in abstr_test_pmid_sents_refined:
    key = list(item.keys())[0]
    docid = item[key]['document_id']
    sent = item[key]['sent']
    offset = item[key]['sent_spans'][0]
...
json.dump(abstr_train_pmid_tokens, open("temp/abstr_train_pmid_tokens.json", "w", encoding="utf-8"))
json.dump(abstr_test_pmid_tokens, open("temp/abstr_test_pmid_tokens.json", "w", encoding="utf-8"))
