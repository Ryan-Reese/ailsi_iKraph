...
ErrorCnt = 0
ErrorCnt2 = 0
for item in tqdm(sentTokens, desc="labelling tokens"):
    tokens = item["tokens"]
    spans = item["spans"]
    ...
    for cnt, [s, e] in enumerate(spans):
        ...
        if entityDic[docID][cur_cnt][0] >= e:
            ...
            if entityDic[docID][cur_cnt][1] == e:
                ...
                print(
                    ErrorCnt,
                    ": Tokenization error 1: ",
                    docID,
                    s,
                    e,
                    tokens[cnt],
                    entityDic[docID][cur_cnt][0],
                    entityDic[docID][cur_cnt][1],
                )
                ErrorCnt = ErrorCnt + 1
                docCnt[docID] = docCnt[docID] + 1
                cur_cnt = docCnt[docID]
            ...
            if entityDic[docID][cur_cnt][1] > e:
                ...
                if (
                    prev_label == "B-" + entityDic[docID][cur_cnt][2]
                    or prev_label == "I-" + entityDic[docID][cur_cnt][2]
                ):
                ...
                if entityDic[docID][cur_cnt][1] <= s:
                    ...
                    if (
                        cur_cnt < len(entityDic[docID])
                        and entityDic[docID][cur_cnt][0] == s
                    ):
                        ...
                        cur_cnt < len(entityDic[docID])
                        and entityDic[docID][cur_cnt][0] < e
                    ):
