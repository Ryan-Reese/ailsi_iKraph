...
def get_pmids(dataset):
    ...
    with open(dataset, "r", encoding="utf-8") as f:
        for line in tqdm(f, desc="getting PMIDs from dataset"):
...
def get_pmcids(pmids):
    ...
    for i in trange(0, len(pmids), 200, desc="getting PMCIDs for PMIDs"):
        if i + 200 <= len(pmids):
...
def get_full_text(pmcids):
        ...
        for art in soup.find_all("article"):
            if art.find("body"):
                ...
                if body.find("sec", attrs={"sec-type": "supplementary-material"}):
                    body.find(
                        "sec", attrs={"sec-type": "supplementary-material"}
                    ).decompose()
                ...
                fulltext = re.sub(
                    r"\\documentclass.*?\$\$\\end\{document\}", "", fulltext
                )
                fulltext_data[pmid] = {
                    "pmid": pmid,
                    "pmcid": f"PMC{pmcid}",
    ...
    for i in trange(0, len(pmcids), 200, desc="getting full text for PMCIDs"):
        if i + 200 <= len(pmcids):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Get full text for articles in litcoin training and test data"
    )
    parser.add_argument(
        "--input_file",
        "-i",
        type=str,
        required=True,
        help="Path and name of the file of litcoin training or test data, in json format",
    )
    parser.add_argument(
        "--output_file",
        "-o",
        type=str,
        required=True,
        help="Path and name of the file for saving full text, in json format",
    )
