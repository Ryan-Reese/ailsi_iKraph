...
def find_last_checkpoint(path):
    checkpoint = sorted(
        [int(x.split("/")[-1].split("-")[-1]) for x in glob.glob(path + "/checkpoint*")]
    )
...
if __name__ == "__main__":
    ...
    ensemble_list = []
    for M in tqdm(glob.glob(model_path + "/[rR]o*a*"), desc="ranking models"):
        M_name = M.split("/")[-1]
        ...
        if "RS" in M_name:
            ...
            e = find_top_checkpoints(
                M + "/checkpoint-" + last_checkpoint + "/trainer_state.json"
            )
            for checkpoint in e:
                ensemble_list.append(
                    result_path + "/" + M_name + "/checkpoint-" + str(checkpoint[0])
                )
